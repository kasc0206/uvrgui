...
class SeperateAttributes:
        ...
        if self.is_gpu_conversion >= 0:
            if mps_available:
                ...
                if cuda_available:  # and not self.is_use_opencl:
                    self.device = (
                        CUDA_DEVICE if not device_prefix else f"{device_prefix}:{self.device_set}"
                    )
    ...
    def write_audio(self, stem_path: str, stem_source, samplerate, stem_name=None):
        ...
        def save_voc_split_instrumental(stem_name, stem_source, is_inst_invert=False):
            ...
            inst_stem_path_name = (
                LEAD_VOCAL_STEM_I if stem_name == LEAD_VOCAL_STEM else BV_VOCAL_STEM_I
            )
        ...
        def deverb_vocals(stem_path: str, stem_source):
            ...
            stem_source_deverbed, stem_source_2 = vr_denoiser(
                stem_source, self.device, is_deverber=True, model_path=self.DEVERBER_MODEL
            )
        ...
        is_bv_model_lead = (
            ...
            and self.is_vocal_split_model
            and stem_name == LEAD_VOCAL_STEM
        )
        ...
        is_no_vocal_save = (
            ...
            and (
                stem_name == VOCAL_STEM
                or stem_name == BV_VOCAL_STEM
                or stem_name == LEAD_VOCAL_STEM
            )
            or is_bv_model_lead
        )
        is_not_ensemble = not self.is_ensemble_mode or self.is_vocal_split_model
        is_do_not_save_inst = (
            self.is_save_vocal_only and self.is_sec_bv_rebalance and stem_name == INST_STEM
        )
                ...
                if self.is_save_inst_vocal_splitter and not self.is_save_vocal_only:
                    ...
                    if is_bv_rebalance_lead:
                        save_voc_split_instrumental(
                            LEAD_VOCAL_STEM, bv_rebalance_lead_source, is_inst_invert=True
                        )
...
class SeperateMDX(SeperateAttributes):
    def seperate(self):
        ...
        if self.primary_model_name == self.model_basename and isinstance(
        ...
        if not self.is_primary_stem_only:
            ...
            if not isinstance(self.secondary_source, np.ndarray):
                ...
                self.secondary_source = (
                    spec_utils.invert_stem(raw_mix, source)
    ...
    def demix(self, mix, is_match_mix=False):
        ...
        step = (
            self.chunk_size - self.n_fft if overlap == DEFAULT else int((1 - overlap) * chunk_size)
        )
        ...
        if self.is_denoise_model and not is_match_mix:
            if NO_STEM in self.primary_stem_native or self.primary_stem_native == INST_STEM:
                ...
                source = org_mix - vr_denoiser(
                    org_mix - source, self.device, model_path=self.DENOISER_MODEL
                )
            else:
    ...
    def seperate(self):
        ...
        if (
            ...
            for stem in stem_list:
                primary_stem_path = os.path.join(
                    self.export_path, f"{self.audio_file_base}_({stem}).wav"
                )
        ...
        if (
            ...
            if not self.is_primary_stem_only:
                ...
                def secondary_save(sec_stem_name, source, raw_mixture=None, is_inst_mixture=False):
                    ...
                    secondary_stem_path = os.path.join(
                        self.export_path, f"{self.audio_file_base}_({sec_stem_name}).wav"
                    )
                    ...
                    self.write_audio(
                        secondary_stem_path, secondary_source, samplerate, stem_name=sec_stem_name
                    )
...
class SeperateVR(SeperateAttributes):
    ...
    def loading_mix(self):
        ...
        for d in range(bands_n, 0, -1):
            ...
            if d == bands_n:  # high-end band
                X_wave[d], _ = librosa.load(
                    audio_file, bp["sr"], False, dtype=np.float32, res_type=wav_resolution
                )
        ...
        X_spec = spec_utils.combine_spectrograms(
            X_spec_s, self.mp, is_v51_model=self.is_vr_51_model
        )
...
def prepare_mix(mix: str | np.ndarray) -> np.ndarray:
...
def save_format(audio_path: str, save_format: str, mp3_bit_set: str) -> None:
    ...
    if not save_format == WAV:
        ...
        if save_format == MP3:
            ...
            try:
                musfile.export(
                    audio_path_mp3, format="mp3", bitrate=mp3_bit_set, codec="libmp3lame"
                )
...
def pitch_shift(mix: np.ndarray) -> np.ndarray:
    new_sr = 31183
...
def list_to_dictionary(lst: list) -> dict:
...
def loading_mix(X: np.ndarray, mp: int) -> np.ndarray:
    ...
    for d in range(bands_n, 0, -1):
        ...
        if d == bands_n:  # high-end band
            ...
            X_wave[d] = librosa.resample(
                X_wave[d + 1], mp.param["band"][d + 1]["sr"], bp["sr"], res_type=wav_resolution
            )
