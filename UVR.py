...
class ModelData:
    def __init__(
        ...
        self.overlap = (
            float(root.overlap_var.get()) if not root.overlap_var.get() == DEFAULT else 0.25
        )
        self.overlap_mdx = (
        ...
        if self.process_method == VR_ARCH_TYPE:
            ...
            self.batch_size = (
                1 if root.batch_size_var.get() == DEF_OPT else int(root.batch_size_var.get())
            )
        ...
        if self.is_vocal_split_model and self.model_status:
            ...
            if self.is_bv_model:
                primary = (
                    BV_VOCAL_STEM if self.primary_stem_native == VOCAL_STEM else LEAD_VOCAL_STEM
                )
            else:
                primary = (
                    LEAD_VOCAL_STEM if self.primary_stem_native == VOCAL_STEM else BV_VOCAL_STEM
                )
    ...
    def ensemble_outputs(
        ...
        if is_4_stem:
            ...
            if is_inst_mix:
                ...
                algorithm = (
                    self.primary_algorithm if stem == PRIMARY_STEM else self.secondary_algorithm
                )
                stem_tag = (
                    self.ensemble_primary_stem
        ...
        audio_file_output = (
            f"{self.is_testing_audio}{audio_file_base}{self.chosen_ensemble}_({stem_tag})"
        )
...
class AudioTools:
    ...
    def align_inputs(
        self, audio_inputs, audio_file_base, audio_file_2_base, command_Text, set_progress_bar
    ):
        audio_file_base = f"{self.is_testing_audio}{audio_file_base}"
        audio_file_2_base = f"{self.is_testing_audio}{audio_file_2_base}"

        aligned_path = os.path.join(
            "{}".format(self.main_export_path), "{}_(Aligned).wav".format(audio_file_2_base)
        )
        inverted_path = os.path.join(
            "{}".format(self.main_export_path), "{}_(Inverted).wav".format(audio_file_base)
        )
    ...
    def match_inputs(self, audio_inputs, audio_file_base, command_Text):
        ...
        match.process(
            ...
            reference=reference,
            results=[
                match.save_audiofile(save_path, wav_set=self.wav_type_set),  # type: ignore[attr-defined]
            ],
        )
...
class ListboxBatchFrame(tk.Frame):
    def __init__(self, master=None, name="Listbox", command=None, image_sel=None, img_mapper=None):
        ...
        self.label = tk.Label(
            self, text=name, font=(MAIN_FONT_NAME, FONT_SIZE_5), foreground=FG_COLOR
        )
    ...
    def update_displayed_index(self, inputs=None, is_acc_dupe=True):
        ...
        for i in range(len(self.path_list)):
            ...
            self.basename_to_path[basename] = self.path_list[
                i
...
class ComboBoxEditableMenu(ttk.Combobox):
    def __init__(
        self, master=None, pattern=None, default=None, width=None, is_stay_disabled=False, **kw
    ):
...
class MainWindow(_MainWindowBase):  # type: ignore[misc]
    ...
    def menu_title_LABEL_SET(self, frame, text, width=35):
        return ttk.Label(
            master=frame,
            text=text,
            font=(SEC_FONT_NAME, FONT_SIZE_5, "underline"),
            justify="center",
            foreground="#13849f",
            width=width,
            anchor=tk.CENTER,
    ...
    def spacer_label(self, frame):
        return tk.Label(
            frame, text="", font=(MAIN_FONT_NAME, FONT_SIZE_1), foreground="#868687", justify="left"
        ).grid()

    # Ensemble Listbox Functions
    def ensemble_listbox_get_all_selected_models(self):
        return [
            self.ensemble_listbox_Option.get(i) for i in self.ensemble_listbox_Option.curselection()
        ]
    ...
    def ensemble_listbox_clear_and_insert_new(self, model_ensemble_updated):
        return (
            ...
            [
                self.ensemble_listbox_Option.insert(tk.END, models)
                for models in model_ensemble_updated
            ],
        )
    ...
    def fill_filePaths_Frame(self):
        ...
        self.filePaths_musicFile_Open = ttk.Button(
            ...
            command=lambda: (
                OPEN_FILE_func(os.path.dirname(self.inputPaths[0]))
        ...
        self.filePaths_saveTo_Open = ttk.Button(
            ...
            image=self.efile_img,
            command=lambda: (
                OPEN_FILE_func(Path(self.export_path_var.get()))
    ...
    def fill_options_Frame(self):
        ...
        self.chosen_process_method_Option = ComboBoxMenu(
            ...
            command=lambda e: self.selection_action_process_method(
                self.chosen_process_method_var.get(), from_widget=True, is_from_conv_menu=True
            ),
        )
        ...
        self.save_current_settings_Option = ComboBoxMenu(
            ...
            command=lambda e: self.selection_action_saved_settings(
                self.save_current_settings_var.get()
            ),
        )
        ...
        self.mdx_net_model_Option = ComboBoxMenu(
            ...
            command=lambda event: self.selection_action(
                event, self.mdx_net_model_var, is_mdx_net=True
            ),
        )
        ...
        self.is_primary_stem_only_Demucs_Option_place = lambda: (
            self.is_primary_stem_only_Demucs_Option.place(
                x=CHECK_BOX_X,
                y=CHECK_BOX_Y,
        ...
        self.is_primary_stem_only_Demucs_Option_toggle = lambda: (
        ...
        self.is_secondary_stem_only_Demucs_Option_place = lambda: (
            self.is_secondary_stem_only_Demucs_Option.place(
                x=CHECK_BOX_X,
                y=CHECK_BOX_Y,
                width=CHECK_BOX_WIDTH,
                height=CHECK_BOX_HEIGHT,
        ...
        self.is_secondary_stem_only_Demucs_Option_toggle = lambda: (
        ...
        self.ensemble_listbox_Option = tk.Listbox(
            ...
            activestyle="dotbox",
            font=(MAIN_FONT_NAME, FONT_SIZE_1),
            background="#070708",
            exportselection=0,
            relief=tk.SOLID,
            borderwidth=0,
        ...
        self.is_primary_stem_only_Option_toggle = lambda: (
        ...
        self.is_secondary_stem_only_Option_toggle = lambda: (
    ...
    def focus_out_widgets(self, all_widgets, frame):
        for option in all_widgets:
            if type(option) is not ComboBoxEditableMenu:
                option.bind(
                    "<Button-1>", lambda e: (option.focus(), self.combo_box_selection_clear(frame))
                )
    ...
    def bind_widgets(self):
        ...
        all_widgets = (
            ...
            + self.filePaths_Frame.winfo_children()
            + other_items
        )
        ...
        self.options_Frame.bind(
            right_click_button,
            lambda e: (self.right_click_menu_popup(e, main_menu=True), self.options_Frame.focus()),
        )
        self.filePaths_musicFile_Entry.bind(
            right_click_button,
            lambda e: (self.input_right_click_menu(e), self.filePaths_musicFile_Entry.focus()),
        )
        self.filePaths_musicFile_Entry.bind(
            "<Button-1>",
            lambda e: (
    ...
    def error_dialoge(self, message):
        ...
        messagebox.showerror(
            master=self,  # type: ignore[call-overload]
            title=message[0],
            message=message[1],
            parent=root,
        )
        ...
        for model in stem_check:
            if is_multi_stem:
                ...
                if is_check_vocal_split:
                    model_name = (
                        None
                        if model.is_karaoke or not model.vocal_split_model
                        else model.model_basename
                    )
                else:
        ...
        def copy_help_hint(event):
            if self.help_hints_var.get():
                ...
                right_click_menu.add_command(
                    label="复制帮助提示文本", command=right_click_menu_copy_hint
                )
    ...
    def input_right_click_menu(self, event):
        ...
        right_click_menu = tk.Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
        right_click_menu.add_command(
            label="查看所有输入", command=lambda: self.check_is_menu_open(INPUTS_MENU)
        )
    ...
    def cached_source_model_list_check(self, model_list: List[ModelData]):
        ...
        self.demucs_secondary_model_names = [
            ...
            and model.secondary_model is not None
            else None
            for model in model_list
        ]
    ...
    def verify_audio(self, audio_file, is_process=True, sample_path=None):
        ...
        for i in audio_file:
            if os.path.isfile(i):
                try:
                    librosa.load(i, duration=3, mono=False, sr=44100) if type(
                        sample_path
                    ) is not str else self.create_sample(i, sample_path)
                    is_good = True
                except Exception as e:
    ...
    def right_click_menu_popup(self, event, text_box=False, main_menu=False):
        ...
        def add_text_edit_options(menu):
            ...
            menu.add_command(
                label="粘贴", command=lambda: self.right_click_menu_paste(text_box=text_box)
            )
            menu.add_command(
                label="删除", command=lambda: self.right_click_menu_delete(text_box=text_box)
            )
        ...
        if not main_menu:
            ...
            if not self.is_menu_settings_open:
                right_click_menu.add_command(
                    label="附加设置", command=lambda: self.menu_settings(select_tab_2=True)
                )
            ...
            if self.error_log_var.get():
                right_click_menu.add_command(
                    label="错误日志", command=lambda: self.check_is_menu_open(ERROR_OPTION)
                )
    ...
    def menu_placement(
        ...
        window.geometry(
            "+%d+%d" % (root_location_x + menu_offset_x, root_location_y + menu_offset_y)
        )
        ...
        def right_click_menu(event):
            help_hints_label = "Enable" if not self.help_hints_var.get() else "Disable"
            help_hints_bool = True if not self.help_hints_var.get() else False
            right_click_menu = tk.Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
            if is_help_hints:
                right_click_menu.add_command(
                    label=f"{help_hints_label} Help Hints",
    ...
    def adjust_widget_widths(self, frame):
        ...
        resize_widget(
            [widget for widget in frame.winfo_children() if isinstance(widget, tk.Button)]
        )
        resize_widget(
            [widget for widget in frame.winfo_children() if isinstance(widget, ttk.Combobox)]
        )
    ...
    def menu_tab_control(self, toplevel, ai_network_vars, is_demucs=False, is_mdxnet=False):
        ...
        def on_tab_selected(event):
            ...
            load_screen = False
            if (
                event.widget.tab("current", option="text") == "Secondary Model"
                and not self.tab2_loaded
            ):
                tab = tab2
            ...
            if load_screen:
                ...
                loading_label = ttk.Label(
                    tab, text="正在更新模型列表...", font=Font(family=MAIN_FONT_NAME, size=14)
                )
                loading_label.place(
    ...
    def menu_view_inputs(self):
        ...
        def selected_files(is_remove=False):
            if not self.thread_check(self.active_processing_thread):
                items_list = [
                    input_files_listbox_Option.get(i)
                    for i in input_files_listbox_Option.curselection()
                ]
                ...
                [
                    input_files_listbox_Option.delete(
        ...
        def right_click_menu(event):
            right_click_menu = tk.Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
            right_click_menu.add_command(
                label="仅移除选中项", command=lambda: selected_files(is_remove=True)
            )
            right_click_menu.add_command(
                label="仅保留选中项", command=lambda: selected_files(is_remove=False)
            )
            right_click_menu.add_command(
                label="清除所有输入", command=lambda: input_options(is_select_inputs=False)
            )
            ...
            if is_play_file_var.get():
                ...
                right_click_menu_sub.add_command(
                    ...
                    command=lambda: (
                        input_files_listbox_Option.bind(
                            "<Double-Button>", lambda e: pop_open_file_path(is_play_file=True)
                        ),
        ...
        if not OPERATING_SYSTEM == "Linux":
            ttk.Button(
                menu_view_inputs_Frame, text=SELECT_INPUTS, command=lambda: input_options()
    ...
    def menu_batch_dual(self):
        ...
        def clear_all(event, lb):
            ...
            if selected:
                right_click_menu.add_command(
                    label="打开位置", command=lambda: open_selected_path(lb)
                )
                right_click_menu.add_command(
                    label="打开文件", command=lambda: open_selected_path(lb, is_play_file=True)
                )
    ...
    def menu_settings(self, select_tab_2=False, select_tab_3=False):  # **
        ...
        select_Option.update_dropdown_size(
            OPTION_LIST,
            "menuchoose",
            command=lambda e: (self.check_is_menu_open(self.main_menu_var.get()), close_window()),
        )
        ...
        _current_label = next(
            (l["label"] for l in _lang_opts if l["code"] == _current_lang), _lang_opts[0]["label"]
        )
        ...
        if not is_macos and self.is_gpu_available:
            ...
            def gpu_list_options():
                return self.loop_gpu_list(
                    device_set_Option, "gpudevice", self.cuda_device_list
        ...
        self.download_progress_info_Label = tk.Label(
            ...
            font=(MAIN_FONT_NAME, FONT_SIZE_2),
            foreground=FG_COLOR,
            borderwidth=0,
        )
    ...
    def menu_advanced_vr_options(self):  # **
        ...
        def toggle_post_process():
            return (
                self.post_process_threshold_Option.configure(state=READ_ONLY)
    ...
    def menu_help(self):  # **
        ...
        def section_title_Label(place, frame, text, font_size=FONT_SIZE_4):
            return tk.Label(
                master=frame,
                text=text,
                font=(MAIN_FONT_NAME, font_size, "bold"),
                justify="center",
                fg="#F4F4F4",
        ...
        def description_Label(place, frame, text, font=FONT_SIZE_2):
            return tk.Label(
                master=frame, text=text, font=(MAIN_FONT_NAME, font), justify="center", fg="#F6F6F7"
            ).grid(row=place, column=0, padx=0, pady=MENU_PADDING_4)
        ...
        def credit_label(place, frame, text, link=None, message=None, is_link=False, is_top=False):
            if is_top:
                thank = tk.Label(
                    master=frame,
                    text=text,
                    font=(MAIN_FONT_NAME, FONT_SIZE_3, "bold"),
                    justify="center",
                    fg="#13849f",
                )
            else:
                thank = tk.Label(
                    master=frame,
                    text=text,
                    font=(MAIN_FONT_NAME, FONT_SIZE_3, "underline" if is_link else "normal"),
                    justify="center",
                    fg="#13849f",
                )
        ...
        def Link(place, frame, text, link, description, font=FONT_SIZE_2):
            link_label = tk.Label(
                master=frame,
                text=text,
                font=(MAIN_FONT_NAME, FONT_SIZE_4, "underline"),
                foreground=FG_COLOR,
                justify="center",
                cursor="hand2",
            )
        ...
        def right_click_menu(event):
            ...
            right_click_menu.add_command(
                label="返回设置菜单",
                command=lambda: (self.menu_help_close_window(), self.check_is_menu_settings_open()),
            )
            right_click_menu.add_command(
                label="退出窗口", command=lambda: self.menu_help_close_window()
            )
        ...
        section_title_Label(place=0, frame=credits_Frame, text="UVR 核心开发团队")
        ...
        appplication_license_Label = tk.Label(
            ...
            font=(MAIN_FONT_NAME, FONT_SIZE_6, "bold"),
            justify="center",
            fg="#f4f4f4",
        )
    ...
    def menu_error_log(self):  #
        ...
        copy_text_Label = tk.Label(
            ...
            font=(MAIN_FONT_NAME, FONT_SIZE_0),
            justify="center",
            fg="#f4f4f4",
        )
    ...
    def menu_manual_downloads(self):
        ...
        def get_links():
            ...
            self.menu_sub_LABEL_SET(
                manual_downloads_link_Frame, SELECTED_MODEL_PLACE_PATH_TEXT
            ).grid(row=MAIN_ROW + 2, column=0, padx=0, pady=MENU_PADDING_4)
            ttk.Button(
                manual_downloads_link_Frame,
        ...
        manual_downloads_menu_select_Option["menu"].add_cascade(
            label="VR 模型", menu=manual_downloads_menu_select_VR_Option
        )
        manual_downloads_menu_select_Option["menu"].add_cascade(
            label="MDX-Net 模型", menu=manual_downloads_menu_select_MDX_Option
        )
        manual_downloads_menu_select_Option["menu"].add_cascade(
            label="Demucs 模型", menu=manual_downloads_menu_select_DEMUCS_Option
        )

        for model_selection_vr in vr_download_list.keys():
            if not os.path.isfile(
                os.path.join(VR_MODELS_DIR, vr_download_list[model_selection_vr])
            ):
    ...
    def pop_up_save_current_settings(self):
        ...
        settings_save_Button = ttk.Button(
            ...
            text=SAVE_TEXT,
            command=lambda: (
                save_func() if validation(settings_save_var.get()) else invalid_message()
            ),
        )
    ...
    def pop_up_save_current_settings_sub_json_dump(self, settings_save_name: str):
        ...
        if settings_save_name:
            ...
            with open(
                os.path.join(SETTINGS_CACHE_DIR, f"{settings_save_name}.json"), "w"
            ) as outfile:
    ...
    def pop_up_user_code_input(self):
        ...
        user_code_confrim_Button = ttk.Button(
            user_code_Frame,
            text=CONFIRM_TEXT,
            command=lambda: self.download_validate_code(
    ...
    def pop_up_change_model_defaults(self, top_window):
        ...
        def change_default():
            model_data = self.assemble_model_data(
                model=change_model_defaults_var.get(), arch_type=ENSEMBLE_CHECK, is_change_def=True
            )[0]
    ...
    def pop_up_set_vocal_splitter(self, top_window):
        ...
        try:
            ...
            def deverb_opt_toggle():
                return (
                    enable_deverb_opt() if self.is_deverb_vocals_var.get() else disable_deverb_opt()
                )
    ...
    def pop_up_mdx_model(self, mdx_model_hash, model_path):
        ...
        try:
            ...
            # self.error_log_var.set(f'{type(e).__name__}: "{e}"\n{"".join(traceback.format_tb(e.__traceback__))}"')
    ...
    def pop_up_save_ensemble(self):
        ...
        if len(self.ensemble_listbox_get_all_selected_models()) <= 1:
            ...
            mdx_param_set_Button = ttk.Button(
                ...
                text=SAVE_TEXT,
                command=lambda: (
                    save_func() if validation(ensemble_save_var.get()) else invalid_message()
                ),
            )
            mdx_param_set_Button.grid(pady=MENU_PADDING_1)
    ...
    def pop_up_save_ensemble_sub_json_dump(self, selected_ensemble_model, ensemble_save_name: str):
        ...
        if ensemble_save_name:
            ...
            with open(
                os.path.join(ENSEMBLE_CACHE_DIR, f"{ensemble_save_name}.json"), "w"
            ) as outfile:
                outfile.write(saved_data_dump)
    ...
    def deletion_list_fill(
        ...
        def refresh_menu(remove=None):
            selection_list = (
                self.last_found_ensembles
                if menu_name == "deleteensemble"
                else self.last_found_settings
            )
            main_var = (
                self.chosen_ensemble_var
                if menu_name == "deleteensemble"
                else self.save_current_settings_var
            )
        ...
        def online_check():
            ...
            try:
                ...
                if self.lastest_version == current_patch and not is_start_up:
                    ...
                    is_beta_version = (
                        True
                        if self.lastest_version == PREVIOUS_PATCH_WIN
    ...
    def download_list_fill(self, model_type=ALL_TYPES):
        ...
        def configure_combobox(
            combobox: ComboBoxMenu, values: list, variable: tk.StringVar, arch_type, name
        ):
                ...
                if is_update_app:
                    ...
                    if self.select_download_var.get() == DEMUCS_ARCH_TYPE and is_demucs_newer:
                        for model_num, model_data in enumerate(
                            self.download_demucs_newer_models, start=1
                        ):
                            self.download_progress_info_var.set(
                                "{} {}/{}...".format(
                                    DOWNLOADING_ITEM,
    ...
    def update_available_models(self):
        ...
        def loop_directories(
            option_menu: ComboBoxMenu, option_var, model_list, model_type, name_mapper=None
        ):
            current_selection = option_menu.get()
            option_list = (
                [fix_name(file_name, name_mapper) for file_name in model_list]
                if name_mapper
                else model_list
            )
            sorted_options = natsort.natsorted(option_list)
            option_list_option_menu = (
                sorted_options + [OPT_SEPARATOR, DOWNLOAD_MORE]
                if self.is_online
                else sorted_options
            )
            ...
            return tuple(
                f"{model_type}{ENSEMBLE_PARTITION}{model_name}" for model_name in sorted_options
            )
    ...
    def update_main_widget_states(self):
        ...
        def stem_save_options():
            place_widgets(
                self.is_primary_stem_only_Option_place, self.is_secondary_stem_only_Option_place
            )
        ...
        def no_ensemble_shared():
            place_widgets(
                self.save_current_settings_Label_place, self.save_current_settings_Option_place
            )
        ...
        if self.chosen_process_method_var.get() == DEMUCS_ARCH_TYPE:
            ...
            if not self.demucs_model_var.get() == CHOOSE_MODEL:
                ...
                self.demucs_stems_Option.command(
                    lambda e: self.update_stem_checkbox_labels(
    ...
    def selection_action_process_method(
        self, selection, from_widget=False, is_from_conv_menu=False
    ):
    ...
    def process_end(self, error=None):
        ...
        if error:
            ...
            confirm = messagebox.askyesno(
                parent=root, title=ERROR_OCCURED[0], message=error_message_box_text
            )
        ...
        try:
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                def set_progress_bar(step, inference_iterations=0):
                    return self.process_update_progress(
    ...
    def check_only_selection_stem(self, checktype):
        ...
        stem_primary_bool = (
            self.is_primary_stem_only_Demucs_var.get()
            if is_demucs
            else self.is_primary_stem_only_var.get()
        )
        ...
        try:
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                if self.verify_audio(audio_file):
                    ...
                    self.command_Text.write(
                        f"\n{error_text_console}"
                    ) if inputPath_total_len >= 2 else None
                ...
                for current_model_num, current_model in enumerate(model, start=1):
                    ...
                    audio_file_base = (
                        f"{file_num}_{os.path.splitext(os.path.basename(audio_file))[0]}"
                    )
                    audio_file_base = (
                        audio_file_base
                    ...
                    process_data = {
                        ...
                        if self.ensemble_main_stem_var.get()
            ...
            self.error_log_var.set(
                "{}{}".format(
                    error_text(self.chosen_process_method_var.get(), e), self.get_settings_list()
                )
            )
            self.command_Text.write(f"\n\n{PROCESS_FAILED}")
            self.command_Text.write(time_elapsed())
    ...
    def load_to_default_confirm(self):
        ...
        if (
            self.pre_proc_model_toggle is not None
            and self.is_open_menu_advanced_demucs_options.get()
        ):
    ...
    def load_saved_settings(
        self, loaded_setting: dict, process_method=None, is_default_reset=False
    ):
